...
def correlations(data, columnsArray, no_rem_col, Val, xVal, yVal):
    ...
    data, miss_cols = missing_val_impute(data, columnsArray=columnsArray, rm_cols=rm_cols)
    print("Data info after missing_val_impute()")
    print(data.info())
    sub= list(filter(lambda x: x not in rm_cols and x in xVal and x in data.columns, Val))
    sub.append(yVal)
    cat_data = data.select_dtypes(include=['category']).copy()
    num_data = data.select_dtypes(include=['number']).copy()
    if (num_data.shape[1]>0):
        ...
        rm_cols = list(filter(lambda x: x in rm_cols or x in col, cols))
        data2, ob_t = transformation(data)
    else:
        data2 = data
        obj_t = "No Numerical"
    print("Data info after missing_val_impute()")
...
#    data = pd.concat(frames, axis=1)
    ...
    return ent_cor,chisq_dependency,data2,rm_cols, miss_cols, obj_t, sub
...
def correlations_cluster(data, columnsArray, no_rem_col,Val):
    # data type conversion and deleting missing values 
    #chisq_dependency = []
    #ent_cor = []    
    cols = data.columns                                                           ### Changes Made
    ...
    if (num_data.shape[1]>0):
        #temp_dat = data.reindex(columns=sub)
        data1, obj_t, col = transformation(data)
        rm_cols = list(filter(lambda x: x in rm_cols or x in col, cols))
        data2= data1
    else:
        data2 = data
...
#    data = pd.concat(frames, axis=1)
    '''
    return data2,rm_cols, miss_cols, obj_t, sub
...
def variable_importance_h2o(data_tempor, predictors, response_col):
        ...
        print('in Var_imp')
        data_tempor[response_col] = data_tempor[response_col].astype('category')
        data_tempor1 = data_tempor.copy()
                    ...
                    for i in data_tempor.columns:
                        if data_tempor[i].dtypes == 'float' or data_tempor[i].dtypes == 'int':
                            ...
                            try:
                                ...
                                except_convert.append(i)
            except Exception:
                    print('In excpetion of Variable importance')
                    data_tempor = data_tempor1
            hf = h2o.H2OFrame(data_tempor)
            print(data_tempor.dtypes)
            train, valid, test = hf.split_frame(ratios=[.8, .1])    
