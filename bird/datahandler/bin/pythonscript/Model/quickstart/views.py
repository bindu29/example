...
import h2o
h2o.init(nthreads = -1, max_mem_size = '12G',strict_version_check = False)
import json
import ast
##Lin reg
...
@csrf_exempt
def datasnapshot(request):
    print("ddddddddddd")
    query = request.POST.get('query')
    dbName = request.POST.get('dbName')
    password = request.POST.get('password')
    userName = request.POST.get('userName')
    columnsArray = request.POST.get('columnsArray')
    print(columnsArray)
    dbHost = request.POST.get('dbHost')
    # dbPort = request.POST.get('dbPort')
    columnList =request.POST.get('columnList')
    print(columnList)
    pool = ThreadPool()
...
def linearRegression(request):
    ...
    parametersObj= request.POST.get('parametersObj')
    print(parametersObj)
    pool = ThreadPool()
                    ...
                    if yValue not in list(rm_cols) and yValue not in drop_columns:
                        ...
                        if data2.shape[1]>1 and len(data2[yValue].unique())>1:
                            if(data[yValue].dtypes == 'float') or (data[yValue].dtypes == 'int64'):
                                ...
                                x = list(filter(lambda x: x in num_data.columns and x in xValues and x not in drop_columns, test.columns))
                                if yValue not in x:
                                    x.append(yValue)
                                ...
                                print('2nd inverse')
                                test_pred_inv = transformation_inv(pd.DataFrame(test_predY[x]),temp)
...
def Decisiontree(request):
    ...
    yValue = ast.literal_eval(yValue)
    yValue = yValue[0]
    fitParameter =request.POST.get('fitParameter');
    controlParameter =request.POST.get('controlParameter');
    pool = ThreadPool()
...
def Forecasting(request):
    ...
    jsonstr = HttpResponse(json.dumps(return_val,default=default), content_type="application/json")
    return jsonstr
        ...
        if len(data[yValue].unique())>30:
            ...
            if len(data[yValue].unique())>30:
                if data[yValue].dtypes == 'datetime64[ns]':
                    output = forecasting_model(data,variable_col = xValues[0],date_col = yValue, model = str(parametersObj['forecastingfamily']), independentVariables='',test_split = 0.25, yValue = yValue, xValues = xValues[0], Objt = obj_t, no_of_periods_to_forecast=int(parametersObj['no_of_periods_to_forecast']), columnsArray_e = columnsArray_e)
                    try:
...
def sentiment_analysis(request):
    ...
    col = col[0]
    pool = ThreadPool()
    async_result = pool.apply_async(sentimentThread, (query,dbName,password,userName,columnsArray,dbHost,columnList,col))
    return_val = async_result.get()
    jsonstr = HttpResponse(json.dumps(return_val), content_type="application/json")
    return jsonstr
...
def sentimentThread(query,dbName,password,userName,columnsArray,dbHost,columnList,col,sentiment=False):
    ...
    labelled_data['sentiment'] = labelled_data['processed_text'].apply(sentimentscore)
    print(labelled_data['sentiment'])
    labelled_data['pos_neg'] = ['positive' if x >=0 else 'negative' for x in labelled_data['sentiment']] #labelled_data[labelled_data['sentiment']==0]]
...
# Faster way to get a sentence clubbed as a trigram/bigram
    ...
    positive=df.loc[text['sentiment']>= 0]
    ...
    for i in range(0,len(positive[col])):
        ...
        negative=df.loc[text['sentiment'] <0]
        ...
        listed=[neg_wordfreq,pos_wordfreq,df_document_topic_json,df_topic_keywords_json,wordfreq]
        return(listed)
...
def Smart_inThread(query,dbName,password,userName,columnsArray,dbHost,columnList,yValue):
    ...
    if data.shape[0]>150:
            print(yValue)
            if len(data[yValue].unique())>1:
                target_var,ent_cor,data,newdata,cat_data,missing_value_df,rm_cols, miss_cols, obj_t, sub,drop_columns= smi.correlations(data, columnsArray=columnsArray_e, no_rem_col=yValue, Val = data.columns)
                data = data.loc[:,~data.columns.duplicated()]
                if data.shape[1]>1:
                    xValues = list(filter(lambda x: x not in rm_cols and x in data.columns, data.columns))
                    data_x_unique = data[xValues]
                ...
                if data.shape[1]>1:
                    ...
                    print('inside imp shape')
                    if yValue not in list(rm_cols) and yValue not in drop_columns:
                        xValues = list(filter(lambda x: x not in rm_cols and x not in miss_cols, data.columns))
                        try:
                        ...
                        if data2.shape[1]>1 and len(data2[yValue].unique())>1:
                            if(data[yValue].dtypes == 'float') or (data[yValue].dtypes == 'int64'):
                                ...
                                try:
                                    temp = list(filter(lambda x: x in obj_t, obj_t))
                                    temp1 = list(filter(lambda x: x in obj_t, obj_t))
                                    print(pd.DataFrame(test_trueY[x]))
                                    test_true_inv = smi.transformation_inv(pd.DataFrame(test_trueY[x]),obj_t)
                                ...
                                true_y.index = range(len(true_y))    test_cat_final = []
                                '''
                                ## Final Dat Frame
                                act_pred=true_y['Actual'] - pred_y['Predicted_values']
                                Diff = pd.DataFrame(columns = ['difference_Actual_predicted'],index=range(len(act_pred)))
                                ...
                                outliers1=Diff[Diff["Deviation"]<=-0.7]
                                outliers = pd.concat([outliers,outliers1], axis = 0)
...
def simulation(request):
    ...
    print(type(yValue))
    '''
    columnsArray_e = request.POST.get('columnsArray')
    columnsArray_e = ast.literal_eval(columnsArray_e)
    test = request.POST.get('test')
    test = ast.literal_eval(test)
    simulationdata =request.POST.get('simulationdata')
